package stepDefinitions;

import java.util.Map;

import org.junit.Assert;
import org.openqa.selenium.WebDriver;

import factory.DriverFactory;
import io.cucumber.datatable.DataTable;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import pages.AccountSuccessPage;
import pages.HomePage;
import pages.RegisterPage;
import utils.CommonUtils;

public class RegistrationSteps {
	
	WebDriver driver;
	private RegisterPage rp;
	private AccountSuccessPage asp;
	private CommonUtils cu;
	//private DriverFactory df;

	@Given("User navigates to Register Account page")
	public void user_navigates_to_register_account_page() {
		
		//df = new DriverFactory();
		//driver= df.getDriver();
		driver = new DriverFactory().getDriver(); //this step reduce code
		HomePage hp = new HomePage(driver);
		hp.clickMyAccount();
	    rp =hp.clickRegisterOption();
	    
	    
	}

	@When("User enters the details into below fields")
	public void user_enters_the_details_into_below_fields(DataTable dataTable) {
	    
		
		Map<String, String> dataMap =dataTable.asMap(String.class,String.class);
	    rp.enterFirstName(dataMap.get("firstName"));
	    rp.enterLastName(dataMap.get("lastName"));
	    cu = new CommonUtils();
	    rp.enterEmail(cu.getEmailWithTimeStamp());
	    rp.enterPhoneNumber(dataMap.get("telephone"));
	    rp.enterPassword(dataMap.get("password"));
	    rp.enterConfirmPassword(dataMap.get("password"));
	    
	}
	
	@When("User enters the details into below fields with duplicate email")
	public void user_enters_the_details_into_below_fields_with_duplicate_email(DataTable dataTable) {
	    
		Map<String, String> dataMap =dataTable.asMap(String.class,String.class);
		rp.enterFirstName(dataMap.get("firstName"));
	    rp.enterLastName(dataMap.get("lastName"));
	    rp.enterEmail(dataMap.get("email"));
	    rp.enterPhoneNumber(dataMap.get("telephone"));
	    rp.enterPassword(dataMap.get("telephone"));
	    rp.enterConfirmPassword(dataMap.get("password"));
	    
	}

	@When("User selects Privacy Policy")
	public void user_selects_privacy_policy() {
		
		rp.clickPrivacyPolicyCheckbox();
	}

	@When("User clicks on Continue button")
	public void user_clicks_on_Continue_button() {
		
		asp = rp.clickContinueBtn();
	    
	}

	@Then("User account should get created successfully")
	public void user_account_should_get_created_successfully() {
		
		Assert.assertEquals("Your Account Has Been Created!",asp.getPageHeading());
	    
	}

	@When("User selects Yes for Newsletter")
	public void user_selects_yes_for_newsletter() {
		
		rp.selectYesNewsletterOption();
	    
	}

	@Then("User should get a proper warning about duplicate email")
	public void user_should_get_a_proper_warning_about_duplicate_email() {
	   
		Assert.assertTrue(rp.displayWarningMessage().contains("Warning: E-Mail Address is already registered!"));
	}

	@When("User dont enter any details into fields")
	public void user_dont_enter_any_details_into_fields() {
	    
		rp.enterFirstName("");
	    rp.enterLastName("");
	    rp.enterEmail("");
	    rp.enterPhoneNumber("");
	    rp.enterPassword("");
	    rp.enterConfirmPassword("");
	}


	@Then("User should get proper warning message for every mandatory field")
	public void user_should_get_proper_warning_message_for_every_mandatory_field() {
	    
		Assert.assertTrue(rp.displayWarningMessage().contains("Warning: You must agree to the Privacy Policy!"));
		Assert.assertEquals("First Name must be between 1 and 32 characters!",rp.getFirstNameWarning() );
		Assert.assertEquals("Last Name must be between 1 and 32 characters!", rp.getLastNameWarning());
		Assert.assertEquals("E-Mail Address does not appear to be valid!", rp.getEmailWarning());
		Assert.assertEquals("Telephone must be between 3 and 32 characters!", rp.getTelephoneWarning());
		Assert.assertEquals("Password must be between 4 and 20 characters!", rp.getPasswordWarning());
		
	}
	
    
}
